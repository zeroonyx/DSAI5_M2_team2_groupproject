...
## 4 - Identify Facts
* order item level by seller, measure the date differences
...
* order level, Stores aggregated metrics (total order value, count of items, array of sellers) to prevent metric duplication during reporting.

* Bridge table: 
    * Spatial distance calculations between customer regions and seller regions.
    * dataset contains first 5 zip digits and aggergate the lat and long
...
erDiagram
    ...
    FACT_ORDER_SUMMARY {
        ...
        int order_rating
        float avg_shipping_distance_km
        list seller_keys
    }
    ...
    %% Fact-to-Summary relationship
    FACT_ORDER_FULFILLMENT }o--|| FACT_ORDER_SUMMARY : "rolls up to"
    
    %% Role-Playing Date Relationships
    DIM_DATE ||--o{ FACT_ORDER_FULFILLMENT : "date_roles"

    %% Theme Coloring Styles
    style FACT_ORDER_FULFILLMENT fill:#f3e8ff,stroke:#7c3aed,stroke-width:2px
    style FACT_ORDER_SUMMARY fill:#ede9fe,stroke:#7c3aed,stroke-width:2px,stroke-dasharray: 5 5
    style DIM_CUSTOMER fill:#ccfbf1,stroke:#0d9488,stroke-width:1px
    style DIM_SELLER fill:#ccfbf1,stroke:#0d9488,stroke-width:1px
    style DIM_PRODUCT fill:#ccfbf1,stroke:#0d9488,stroke-width:1px
    style DIM_DATE fill:#ccfbf1,stroke:#0d9488,stroke-width:1px
